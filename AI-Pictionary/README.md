...
6. **AI 猜測**：Gemini 多模態模型（`gemini-2.5-flash`）看圖猜測，回應解析出答案/評分/點評後顯示。
...
### Gemini AI 整合
- 模型：`gemini-2.5-flash`（`gemini-1.5-flash` 已棄用，會回 404）。
- 呼叫：`POST /v1beta/models/gemini-2.5-flash:generateContent?key=<KEY>`，body 含圖片 Base64。
